...
class Completeids extends React.Component {
  constructor(){
    super();
    this.state = {
      data : [],
...
addData(){
  let data1 = fetch('https://randomuser.me/api/?results=1').then((resp)=>{
    resp.json().then((res)=>{
...
getData(){
  let data = fetch('https://randomuser.me/api/?results=50').then((resp)=>{
    resp.json().then((res)=>{
      console.log('parsed json', res)
      this.setState({
        data: res.results
...
<Card className="h-auto p-1 m-2 ml-3">
   <CardImg className="h-25" style={{maxHeight:"220px"}} top src={item.picture.large} alt="alt" onError={(e)=>
